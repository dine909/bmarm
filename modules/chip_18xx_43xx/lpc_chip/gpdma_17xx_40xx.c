...
static uint8_t DMAMUX_Config(uint32_t gpdma_peripheral_connection_number)
{
	if (gpdma_peripheral_connection_number > 15) {
		LPC_SYSCON->DMAREQSEL |= (1 << (gpdma_peripheral_connection_number - 16));
		return gpdma_peripheral_connection_number - 16;
	}
	else {
		LPC_SYSCON->DMAREQSEL &= ~(1 << (gpdma_peripheral_connection_number));
...
void Chip_GPDMA_Init(void)
{
	...
	Chip_SYSCON_PCLKEnable(SYSCON_CLK_GPDMA);
...
{
	Chip_SYSCON_PCLKDisable(SYSCON_CLK_GPDMA);
}
