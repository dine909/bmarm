...
static SYSCON_CLK_T Chip_I2C_DetermineClk(LPC_I2C_Type *I2Cx) {

	SYSCON_CLK_T i2cclk;

	/* Pick clock for uart BASED ON SELECTED uart */
	if (I2Cx == LPC_I2C0) {
		i2cclk = SYSCON_CLK_I2C0;
	}
	else if (I2Cx == LPC_I2C1) {
		i2cclk = SYSCON_CLK_I2C1;
	}
	else {
		i2cclk = SYSCON_CLK_I2C2;
	}
...
{
	/* Enable I2C Clocking */
	Chip_SYSCON_PCLKEnable(Chip_I2C_DetermineClk(I2Cx));
...
void Chip_I2C_DeInit(LPC_I2C_Type *I2Cx)
{
	...
	Chip_SYSCON_PCLKDisable(Chip_I2C_DetermineClk(I2Cx));
}
...
void Chip_I2C_SetClockRate(LPC_I2C_Type *I2Cx, uint32_t clockrate)
{
	if (I2Cx == LPC_I2C0) {
		...
		if (clockrate >= 1000000) {	/* Fast mode plus: 1MHz, high speed 3.4MHz */
			Chip_IOCON_PinMux(0, 27, MD_PLN | MD_HS_ENA | MD_HD_ENA, FUNC1);
			Chip_IOCON_PinMux(0, 28, MD_PLN | MD_HS_ENA | MD_HD_ENA, FUNC1);
		}
		else {					/* standard 100KHz, fast 400KHz */
			Chip_IOCON_PinMux(0, 27, MD_PLN | MD_HS_ENA | MD_HD_DIS, FUNC1);
			Chip_IOCON_PinMux(0, 28, MD_PLN | MD_HS_ENA | MD_HD_DIS, FUNC1);
		}
	}
	else if (I2Cx == LPC_I2C1) {
	...
	IP_I2C_SetClockRate(I2Cx, (Chip_SYSCON_GetPCLKRateFromClk(Chip_I2C_DetermineClk(I2Cx)) / clockrate));
}
