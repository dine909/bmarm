...
void Chip_ENET_Init(void)
{
	...
	Chip_SYSCON_PCLKEnable(SYSCON_CLK_ENET);
	Chip_SYSCON_PeriphReset(SYSCON_RESET_ENET);
...
uint32_t Chip_ENET_FindMIIDiv(uint32_t clockRate)
{
	...
	tmp = Chip_SYSCON_GetCPUCLKRate() / clockRate;
...
void Chip_ENET_DeInit(void)
{
	...
	Chip_SYSCON_PCLKDisable(SYSCON_CLK_ENET);
}
